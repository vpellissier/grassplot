...
The masterfile used is the version 1.23, as found on December 13th 2018.
The sheet in Grassplot 1.23.xlsx containing the LU information is the sheet 'datasets' and was read and saved as a .rds file to ease the process (faster loading)

```{r, echo = T}
df <- readRDS(file.path(path_grassplot, 'Grassplot 1.23_Data.rds'))
df <- df %>%
...
unique_details_lut <- na.omit(setdiff(land_use_details$land_use_details, df$land_use_details))
unique_details_df <- na.omit(setdiff(df$land_use_details, land_use_details$land_use_details))
```
...
if(length(unique_details_df) == 0){
  ...
  for(nb in seq(nrow(land_use_details))){
    df[which(df$land_use_details %in% land_use_details[nb, 'land_use_details']), setdiff(names(land_use_details), 'GrassPlot ID of plot')] <- land_use_details[nb, setdiff(names(land_use_details), 'GrassPlot ID of plot')]
  }
  
  df <- df[,setdiff(names(df), lu_columns)]
  cat('\nA new version of the data sheet of the masterfile have been saved, called Grassplot_data_v1.23.xlsx.')
  cat('\nThis version only contains columns matching the new land-use template.')
  write_xlsx(df, 'Grassplot_data_v1.23.xlsx')
}
