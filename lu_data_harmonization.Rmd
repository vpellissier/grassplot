...
```
  ...
  * `r nb_new0_old1` plots with grazed == 0 & Grazing (1/0) == '1'
  * `r nb_new0_oldprob` plots with grazed == 0 & Grazing (1/0) == 'probably'
...
df %>%
    ...
    pull()
```
  * PL_A contains 6 plots (PL_A_N004_xxxxa) noted as mown in column 95 with Grazing = 'probably'. Without further information, these plots are left as grazed = 0.
##### *Datasets plots with grazed == 1 & Grazing (1/0) == '?':*
```{r echo = F}
df %>%
    filter(`grazed` == 1 & `Grazing (1/0)` == '?')%>%
    distinct(`GrassPlot ID of plot`)%>% 
    pull()
```
    * PL_A contains 7 plots (PL_A_N003_xxxxb) noted as mown/grazed in column 95. These plots are left with grazed == 1
