...
The masterfile used is the version 1.22, as found on December 6th 2018.
The sheet in Grassplot 1.22.xlsx containing the LU information is the sheet 'datasets' and was read and saved as a .rds file to ease the process (faster loading)
...
The columns 99 (mowing frequency) and 101 (grazing intensity) are renamed as `mowing_frequency` and `grazing intensity`:
```{r}
...
The column 105 (Fertilized (1/0)) should be a binary column. Yet, it contains the values `r as.character(na.omit(unique(df[,'Fertilized (1/0)'])))`, meaning that it is an intensity column. The column is thus renamed `fertilization_intensity`
```{r}
names(df)[which(names(df) == 'Fertilized (1/0)')] <- 'fertilization_intensity'
```

The column 104 (Ex arable years (years since last ploughing)) is renamed `years_abandonment`
```{r}
...
**PRELIMINARY NOTE: there are `r ifelse(length(dup_id) > 0, length(dup_id), "no")` duplicated ID in the column `Grassplot ID of plot`.
`r ifelse(length(dup_id) > 0, paste("The following ID are duplicated", dup_id), "")`**
...
The global_land_use lookup table is used to harmonize the column 96 with the new binary columns:
```{r}
df <- df %>%
...
The column is converted to numerical, meaning that we lose the information for now, but it will be retrieved later using the column 97.
...
The verbal descriptors `r non_num_grazing` are replaced by numerical values and the column is converted to numeric:
...
The fertilization_intensity column does not have non-numeric values.  
...
## Abandonment length
The year_abandonment column does not have non-numeric values.
